...
def drawActorswithScores(reprs, fig, color, score, iou, line_width=1):
    ...
    for r,s,u in zip(reprs,score,iou):
        x, y, z = r[0], r[1], r[2]
        yaw = r[4]
        mlab.plot3d([x,x+2*np.cos(yaw)], [y, y], [z,z+2*np.sin(yaw)], color=(1,0,0), tube_radius=None,
                    line_width=line_width, figure=fig)
        b = getBBpts(r)
        ...
        mlab.points3d(b[0, 0], b[0, 1], b[0, 2],color=(1,0,0),line_width=0.5,figure=fig)
        mlab.points3d(b[6, 0], b[6, 1], b[6, 2], color=(0, 1, 0), line_width=0.5, figure=fig)


def nmsProcessing(dets,gts):
    dets = dets[:, :7]
...
for i in range(0,1000):
    ...
    mlab.clf(figure=fig)
    # print(i)
    # f = open(root+str(i)+'.txt','w',encoding='utf-8')
    # _input_all = next(test_iter_all)
    # # Get predictions and gt
    # _, detsEarly, gts = detectForCam(cfg, _input_all, model)
    # detsEarly = np.concatenate(detsEarly)
    # detsEarly = nms(detsEarly, 0.1, 0.01)
    # detsEarly, _, vehicleIDEarly = nmsProcessing(detsEarly, gts)
    # deltasEarly = getDeltas(detsEarly, gts, vehicleIDEarly)
    # for vid, delta in zip(vehicleIDEarly, deltasEarly):
    #     vid = int(vid)
    #     f.write(str(vid))
    ...
    for cam in cameras:
        ...
        #Get ious
        dets, mIOU, vehicleID = nmsProcessing(dets,gts)
        #Get deltas
        deltas = getDeltas(dets,gts,vehicleID)
    #     # Write to file
    ...
    drawActors(groundtruth, fig, (1,1,1), line_width= 2)
    #f.close()
